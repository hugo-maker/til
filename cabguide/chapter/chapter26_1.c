...
// sample program from Chapter26_1
// File chapter26_1.c

/* The program looks for a number of random integers, allocates an array and fills it with numbers between 1 and 500
 * and then loops through all the numbers and figures out the smallest, the biggest, and the average.
...
int main(void)
{
  ...
  // Loop through the now-filled array testing for the random numbers that are biggest, smallest and adding all nubmers together to calculate an average
  for (i = 0; i < aSize; i++)
  {
    total += randomNums[i];
