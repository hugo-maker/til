...
static int make_random(RadixMap * map)
{
  ...
  for (i = 0; i < map->max - 1; i++)
  {
    ...
    check(RadixMap_add(map, key, i) == 0,
        "Failed to add key %u.", key);
  }
...
static int test_search(RadixMap * map)
{
  ...
  for (i = map->end / 2; i < map->end; i++)
  {
    ...
    check(found != NULL, "Did not find %u at %u.", d->data.key, i);
...
static char * test_operations()
{
  ...
  mu_assert(check_order(map), "RadixMap did not stay sorted after search.");
  ...
  while (map->end > 0)
  {
    ...
    mu_assert(RadixMap_delete(map, el) == 0, "Did not delete it.");
    mu_assert(old_end - 1 == map->end, "Wrong size after delete.");

    // test that the end is now the old value, but uint32 max so it traits off
    mu_assert(check_order(map), "RadixMap did not stay sorte after delete.");
  }
