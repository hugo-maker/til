...
/*
 * st_line_buffer の中で、現在文字が格納されている部分のサイズ。
 */
static int st_current_used_size = 0;

/*
 * st_line_buffer の末尾に一文字追加する。
 * 必要とあれば、st_line_buffer の先の領域を拡張する。
 */
static ReadLineStatus add_character(int ch)
{
  /*
   * st_current_used_size は必ず1ずつ増えるので、
   * いきなり抜かれていることはないはず。
   */
  assert(st_current_buffer_size >= st_current_used_size);

  /*
   * st_current_used_size が st_current_buffer_size に追いついたら
   * バッファを拡張する。
   */
...
/*
 * バッファを解放する。
 * プログラム終了時には、malloc()した領域はすべてfree()しておきたい!
 * という場合は、最後にこれを呼べば良い。
 */
