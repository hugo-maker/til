...
/*
 * A safer version that checks for many common errors using
...
int main(int argc, char *argv[])
{
  ...
  check(rc == -1, "sarercopy should fail #1");
  check(to[to_len - 1] == '\0', "String not terminated.");

  rc = safercopy(from_len, from, 0, to);
  check(rc == -1, "sarercopy should fail #2");
  check(to[to_len - 1] == '\0', "String not terminated.");
